...
describe('spar frames', function() {
  describe('About us frame page', function () {
    ...
    before( function(done) {
      var aboutUsLink = document.querySelector("spr-link[path='about us/sections']");
      aboutUsLink.click();
      setTimeout(function(){
      path1 = document.querySelector('spr-route.aboutus-section.path1');
        done()
      },1000);
    });
    ...
    it('should contain routes with content', function() {
      ...
      allValid = Array.from(nodesAboutUs).reduce(function reducer( bool, node ) {
        ...
        if ( node.className === 'aboutus-section src-js' ) {
          nodes.srcJS = node;
        }
        if ( node.className === 'aboutus-section invoked-js') {
          nodes.invokedJS = node;
        }
        if( node.className === 'aboutus-section slot') {
          nodes.slot = node;
        }
